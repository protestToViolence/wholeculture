...
def admin_dashboard():
    ...
    category_percentages = [(category, (count / total_events) * 100) for category, count in category_count]

    max_category = max(category_percentages, key=lambda x: x[1])
    min_category = min(category_percentages, key=lambda x: x[1])

    return render_template('admin_dashboard.html', labels=labels, values=values, months=months, event_counts=event_counts, category_labels=category_labels, category_values=category_values, max_category=max_category, min_category=min_category)
